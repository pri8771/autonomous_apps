...
function open(query, valid=true) {
  ...
  const context = vm.createContext({window,URLSearchParams,document:{getElementById:id=>elements[id]},FormData:class {get(key){return key==='storeUrl' ? 'https://example.com/product' : key==='details' ? 'Check a currency & <value> mismatch' : 'Provided';}}});
  vm.runInContext(plans,context);vm.runInContext(inquiry,context);
  return {elements,window,submit:()=>submit({preventDefault(){}})};
}
